...
for kkk = 1 : length(gamma_vec)
    ...
    freq_relevant = freq(freq_herz >= f_min & freq_herz <= f_max); %Saves only relevant frequencies (20 Hz to 5 Hz)
...
%N4Sid
    ...
    G_n4sid_sys = n4sid(idfrd_Data,n_order,'Ts',Ts,'DisturbanceModel','none',opt);
...
loglog(s(1,1),freq_herz_relevant,abs(G_estimates(freq_herz >= f_min & freq_herz <= f_max,1)),'LineWidth',1.5,'Color', hsv2rgb(0.3,0,0.2));
hold(s(1,1),'on')
...
for kkk = 2 : length(gamma_vec)
    loglog(s(1,1),freq_herz_relevant,abs(G_estimates(freq_herz >= f_min & freq_herz <= f_max,kkk)),'LineWidth',1.3,'Color', hsv2rgb(0.3,1,0.7),'LineStyle','--');
    loglog(s(1,1),freq_herz_relevant,abs(G_estimates(freq_herz >= f_min & freq_herz <= f_max,length(gamma_vec) + kkk)),'LineWidth',1.8,'Color', hsv2rgb(0,1,0.7));
end
hold(s(1,1),'off')


title(s(1,1),{'FF Model: Optimized Result','|G(e^{j\omega})|'})
...
legend(s(1,1), 'Non-Smoothed',  ['Smoothed: \gamma =' num2str(gamma_vec(2))], ['N4SID: \gamma =' num2str(gamma_vec(2)) ', n = ' num2str(n_order)],'Location','Southwest')

s(1,2) = subplot(2,1,2,'Parent',f(1)); %Phase

if(strcmp(experiment,'FB'))
    tol_noise = 1.7*pi
    tol_res = pi;
else
    tol_noise = 50*pi
...
semilogx(s(1,2),freq_herz_relevant,unwrap(angle(G_estimates(freq_herz >= f_min & freq_herz <= f_max,1)),tol_noise)*180/pi,'LineWidth',1.5,'Color', hsv2rgb(1*(1/length(gamma_vec)),0,0.2));
hold(s(1,2),'on')
...
for kkk = 2 : length(gamma_vec)
    semilogx(s(1,2),freq_herz_relevant,unwrap(angle(G_estimates(freq_herz >= f_min & freq_herz <= f_max,kkk)),tol_res)*180/pi,'LineWidth',1.3,'Color', hsv2rgb(0.3,1,0.7),'LineStyle','--');
    semilogx(s(1,2),freq_herz_relevant,unwrap(angle(G_estimates(freq_herz >= f_min & freq_herz <= f_max,length(gamma_vec) + kkk)),tol_res)*180/pi,'LineWidth',1.8,'Color', hsv2rgb(0,1,0.7));
end
hold(s(1,2),'off')
...
legend(s(1,2), 'Non-Smoothed',  ['Smoothed: \gamma =' num2str(gamma_vec(2))], ['N4SID: \gamma =' num2str(gamma_vec(2)) ', n = ' num2str(n_order)],'Location','Southwest')
...
if(sum(~(abs(pole(G_n4sid_sys))<1)) == 0)
   pole(G_n4sid_sys)
